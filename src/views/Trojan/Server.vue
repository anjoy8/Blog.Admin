<template>
  <section>
    ...
    >
      ...
      <el-table-column prop="servername" label="服务器名称" width sortable></el-table-column>
      <el-table-column prop="serveraddress" label="服务器地址" width sortable></el-table-column>
      <el-table-column prop="serverpeer" label="服务器peer(SNI)" width sortable></el-table-column>
      <el-table-column prop="serverport" label="服务器端口" width sortable></el-table-column>
      <el-table-column prop="servertype" label="服务器类型" width sortable>
           <template slot-scope="scope">
           <el-select :disabled="true" v-model="scope.row.servertype" placeholder="请选择服务器类型">
      ...
      <el-table-column prop="serverpath" label="服务器路径" width sortable></el-table-column>
    ...
    >
      <el-form :model="editForm" label-width="200px" :rules="editFormRules" ref="editForm">
        ...
        <el-form-item label="服务器peer(SNI)" prop="serveraddress">
          <el-input v-model="editForm.serverpeer" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="服务器端口" prop="serverport">
          <el-input v-model="editForm.serverport" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="服务器类型" prop="servertype"> 
            <el-select v-model="editForm.servertype" placeholder="请选择服务器类型">
              <el-option key="0" label="普通HTTP" value="0">
              </el-option>
              <el-option key="1" label="WebSocket" value="1">
              </el-option>
        ...
        <el-form-item v-if="editForm.servertype === '1'" label="服务器路径" prop="serverpath">
          <el-input v-model="editForm.serverpath" auto-complete="off"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    handleDel(row) {
      this.$confirm("确认删除吗？", "提示", {}).then(() => {
        var servers = [];
        servers.push(row.id);
        delTrojanServer(servers).then(res => {
          if (res.data.success) {
            this.searchWeChatAccount();
            this.$message({
              message: "删除成功!",
        ...
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            if (this.editType == "add") {
              ...
              //console.log(this.editForm);
              //var postPara = this.editForm;
              updateTrojanServer(this.editForm)
                .then(res => {
                  this.editLoading = false;
                  if (res.data.success) {
                    this.$message({
